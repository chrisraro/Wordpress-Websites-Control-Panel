...
import type { BulkScope } from "@/services/bulk/types";
...
const pluginScope = (over: Partial<Extract<BulkScope, { target: "plugin" }>> = {}): BulkScope => ({
  target: "plugin",
  plugins: testPlugins,
  ...over,
});

const themeScope = (over: Partial<Extract<BulkScope, { target: "theme" }>> = {}): BulkScope => ({
  target: "theme",
  themes: testThemes,
  ...over,
});
...
describe("splitEligible — plugins", () => {
  it("excludes an active plugin from delete, with a reason", () => {
    const s = splitEligible("delete", pluginScope(), ["a/a.php", "b/b.php"]);
  ...
  it("excludes a plugin with no update from update", () => {
    const s = splitEligible("update", pluginScope(), ["a/a.php", "b/b.php"]);
    expect(s.included.map((i) => i.id)).toEqual(["a/a.php"]);
  });

  it("excludes an already-active plugin from activate", () => {
    const s = splitEligible("activate", pluginScope(), ["a/a.php", "b/b.php"]);
    expect(s.included.map((i) => i.id)).toEqual(["b/b.php"]);
  });

  it("excludes an id with no matching plugin in the inventory, with a reason", () => {
    const s = splitEligible("update", pluginScope(), ["ghost/ghost.php"]);
...
describe("splitEligible — themes", () => {
  it("excludes the parent of the active theme from delete", () => {
    const s = splitEligible("delete", themeScope(), ["parent", "spare"]);
  ...
  it("keeps the delete reason from the theme safety gate", () => {
    const s = splitEligible("delete", themeScope(), ["child"]);
    expect(s.included).toEqual([]);
    expect(s.excluded[0].reason).toMatch(/active/i);
  });

  it("excludes every theme from deactivate — themes are switched, not deactivated", () => {
    const s = splitEligible("deactivate", themeScope(), ["spare"]);
    expect(s.included).toEqual([]);
    expect(s.excluded).toEqual([
  ...
  it("excludes an id with no matching theme in the inventory, with a reason", () => {
    const s = splitEligible("update", themeScope(), ["ghost"]);
    expect(s.included).toEqual([]);
...
describe("enqueueBulk", () => {
  ...
  it("inserts one job per eligible item, all sharing the same batch_id", async () => {
    const f = fakeDeps();
    const res = await enqueueBulk(f.deps, "site-1", "user-1", "update", pluginScope(), ["a/a.php", "b/b.php"]);
  ...
  it("produces no jobs for ineligible items", async () => {
    const f = fakeDeps();
    await enqueueBulk(f.deps, "site-1", "user-1", "update", pluginScope(), ["b/b.php"]);
    expect(f.jobsInserted).toHaveLength(0);
  });

  it("returns batchId: null and inserts nothing when nothing is eligible", async () => {
    const f = fakeDeps();
    const res = await enqueueBulk(f.deps, "site-1", "user-1", "update", pluginScope(), ["b/b.php"]);
