...
- **Scheduling:** Supabase `pg_cron` + `pg_net` fires HTTP POSTs (with a shared `CRON_SECRET` header) at Vercel API routes. Rationale: Vercel Hobby crons run at most once/day; pg_cron gives per-minute granularity for free. **pg_cron is the only scheduler — `vercel.json` must declare no `crons` entry.** A once-daily Vercel Cron "backstop" for the nightly enqueue was tried and reverted: `enqueueJob`'s dedupe guard only suppresses a duplicate while the original job is still pending, so a second scheduler firing after the first batch already ran double-enqueues the whole nightly fan-out — every client site's snapshot, security scan, and Wordfence-backed vuln feed refresh, twice in one night. See `docs/ops/scheduling.md`.
- **External services:**
